        ...
        <xsl:for-each select="gmd:distributionInfo/gmd:MD_Distribution|gmd:distributionInfo/*[@gco:isoType='gmd:MD_Distribution']">
          ...
          <xsl:call-template name="complexElementGuiWrapper">
            ...
            <xsl:with-param name="content">
              ...
              <xsl:call-template name="complexElementGuiWrapper">
                <xsl:with-param name="title" select="/root/gui/schemas/iso19139.nap/labels/element[@name='gmd:distributorContact']/label"/>
                <xsl:with-param name="content">
              ...
              <tr>
                <td>
                  <h4 title="{/root/gui/schemas/iso19139.nap/strings/MapResourcesDesc}">
                    ...
                    <xsl:value-of select="/root/gui/schemas/iso19139.nap/strings/MapResources" /></h4>
          ...
          <xsl:otherwise>
            <select class="md" name="{$ref}" size="1" title="{$tooltip}">
              ...
              <option value="eng; CAN">
                ...
                <xsl:value-of select="/root/gui/schemas/iso19139.nap/strings/english" />
              </option>
              ...
              <option value="fra; CAN">
                ...
                <xsl:value-of select="/root/gui/schemas/iso19139.nap/strings/french" />
              </option>
            </select>
          </xsl:otherwise>
        </xsl:choose>
          ...
          <xsl:when test="$edit=true()">
            ...
            <select class="form-control" name="_{gco:CharacterString/geonet:element/@ref}"
              ...
              <optgroup label="{$canLabel}">
                <xsl:for-each select="/root/gui/schemas/iso19139.nap/strings/adminAreas/area[@country='CAN']">
              ...
              <optgroup label="{$usaLabel}">
                <xsl:for-each select="/root/gui/schemas/iso19139.nap/strings/adminAreas/area[@country='USA']">
                  <xsl:variable name="areaEng" select="@eng" />
                  <xsl:variable name="areaFre" select="@fre" />
                  <option>
                    <xsl:attribute name="value"><xsl:value-of select="@eng"/></xsl:attribute>
...
-->
    ...
    <xsl:choose>
      ...
      <xsl:otherwise>
        ...
        <xsl:variable name="textValue">
          <xsl:value-of select="gco:DateTime|gco:Date" /> <xsl:text> (</xsl:text><xsl:value-of select="/root/gui/schemas/iso19139.nap/codelists/codelist[@name='gmd:CI_DateTypeCode']/entry[code = $dateTypeCode]/label "/><xsl:text>)</xsl:text>
        </xsl:variable>
  ...
  <xsl:template mode="iso19139" match="gmd:transferOptions/gmd:MD_DigitalTransferOptions" priority="100">
    ...
    <xsl:if test="count(gmd:onLine) &gt; 0">
      ...
      <p><strong><xsl:value-of select="/root/gui/schemas/iso19139.nap/strings/resources" /></strong></p>
      <ul>
