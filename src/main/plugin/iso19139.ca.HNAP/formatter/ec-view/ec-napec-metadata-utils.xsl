...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template name="parentCollection">
    ...
    <xsl:if test="count($resources/*) &gt; 0">
      <h3><xsl:value-of select="/root/schemas/iso19139.ca.HNAP/strings/ParentCollection"/></h3>
        ...
        <div class="relatedDatasets" id="rlds{position()}">
          ...
          <div style="float:left"> <!-- todo: check here the schema for icon to display -->
            ...
            <xsl:variable name="classificationFilter">
              <xsl:choose>
                <xsl:when test="$schema = 'iso19139.ca.HNAP'">_classificationTheme=<xsl:value-of select="$theme" />&amp;_classificationSubtheme=<xsl:value-of select="$subtheme" />&amp;</xsl:when>
          ...
          <div style="float:right">
            <a class="btn btn-default btn-sm" href="{/root/gui/url}/metadata/{/root/lang}/{id}"><xsl:value-of select="/root/schemas/iso19139.ca.HNAP/strings/View"/></a>
          </div>
  ...
  <xsl:template name="relatedDatasets">
    ...
    <xsl:if test="count($resources/*) &gt; 0">
      <h3><xsl:value-of select="/root/schemas/iso19139.ca.HNAP/strings/Relateddata"/> (<xsl:value-of select="count($resources)" />)</h3>
      <xsl:variable name="guiLang" select="/root/lang" />
      <xsl:for-each select="$resources">
        <!-- Sort first datasets -->
        <xsl:sort select="normalize-space(title/value[@lang=$guiLang])"/>
      ...
      <xsl:if test="count($resources) &gt; 5">
        <a href="javascript:void(jQuery.each(jQuery('.relatedDatasets'),function(){{this.style.display='block'}}))" onclick="this.style.display='none'">
          <xsl:value-of select="concat((count($resources)-5),' ',/root/schemas/iso19139.ca.HNAP/strings/more,'...')" /></a>
      </xsl:if>
    </xsl:if>
  </xsl:template>
  ...
  <xsl:template mode="render-field"
    ...
    <dl>
      <dt>
        <xsl:value-of select="/root/schemas/*[name()=$schema]/strings/Theme"/>
      </dt>
      <dd>
        <a style="text-decoration:underline;"
           href="{/root/gui/url}/search/{/root/lang}?_schema=iso19139.ca.HNAP&amp;_classificationTheme={$mdTheme}"
  ...
  <xsl:template mode="render-field"
    ...
    <dl>
      <dt>
        <xsl:value-of select="/root/schemas/*[name()=$schema]/strings/Subtheme"/>
      </dt>
      <dd>
        <a style="text-decoration:underline;"
           href="{/root/gui/url}/search/{/root/lang}?_schema=iso19139.ca.HNAP&amp;_classificationTheme={$mdTheme}&amp;_classificationSubtheme={$mdSubtheme}"
