...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:gco="http://www.isotc211.org/2005/gco"
  ...
  <xsl:template match="/gmd:MD_Metadata/gmd:identificationInfo/*/gmd:citation/*/gmd:title">
      <title>
        ...
        <xsl:attribute name="lang"><xsl:value-of select="$mainLanguage_ISO639_2B"/></xsl:attribute>
      ...
      <xsl:for-each select="gmd:PT_FreeText/*/gmd:LocalisedCharacterString">
        <title>
          ...
          <xsl:variable name="lang" select="$locales[@id=$localId]/gmd:languageCode/gmd:LanguageCode/@codeListValue"/>
          ...
          <xsl:attribute name="lang"><xsl:value-of select="$lang_ISO639_2B"/></xsl:attribute>
          <xsl:value-of select="."/>
        </title>
      </xsl:for-each>
  </xsl:template>
