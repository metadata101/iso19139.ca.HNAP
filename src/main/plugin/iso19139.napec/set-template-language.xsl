...
<xsl:stylesheet   xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0"
  ...
  <xsl:template match="gmd:descriptiveKeywords">
      <xsl:choose>
        <!-- Don't copy the classification keywords if saved as a template -->
        <xsl:when test="$template = 'y'">
          <xsl:if test="gmd:MD_Keywords[not(string(@id)) or (string(@id) and (@id != 'classification-theme') and (@id != 'classification-subtheme'))]">
            <xsl:copy>
              <xsl:copy-of select="@*" />
