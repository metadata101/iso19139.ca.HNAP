<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
      ...
      <xsl:for-each select="$resources">
        ...
        <div class="relatedDatasets" id="rlds{position()}">
          ...
          <div style="float:right; width:20%">
            <a class="btn btn-default btn-sm btn-block" href="{/root/gui/url}/metadata/{/root/lang}/{id}"><xsl:value-of select="/root/schemas/iso19139.napec/strings/ViewParent"/></a>
            <a class="btn btn-default btn-sm btn-block" style="margin-top: 5px" href="{/root/gui/url}/search/{/root/lang}?_schema={$schema}&amp;{$classificationFilter}parentUuid={id}" title="Search metadata in the collection {normalize-space(title/value[@lang=$guiLang])}"><xsl:value-of select="/root/schemas/iso19139.napec/strings/ViewCollection"/></a>
          </div>
    ...
    <xsl:if test="count($resources/*) &gt; 0">
      ...
      <xsl:if test="count($resources) &gt; 5">
        <a href="javascript:void(jQuery.each(jQuery('.relatedDatasets'),function(){{this.style.display='block'}}))" onclick="this.style.display='none'">
          <xsl:value-of select="concat((count($resources)-5),' ',/root/schemas/iso19139.napec/strings/more,'...')" /></a>
      </xsl:if>
    </xsl:if>
  </xsl:template>
  ...
  <xsl:template name="showMetadataExportIcons">
    ...
    <!-- add pdf link -->
    <xsl:variable name="pdfUrl" select="concat($nodeUrl, $language, '/pdf?uuid=',$muuid, '&amp;fromWorkspace=', $workspace)"/>
