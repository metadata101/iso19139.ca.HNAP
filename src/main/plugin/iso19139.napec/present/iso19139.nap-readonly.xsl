...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template name="iso19139.napecCustomView">
    ...
    <div class="col-md-8 ec-md-detail" itemscope="" itemtype="http://schema.org/Dataset">
      ...
      <h3><xsl:value-of select="/root/gui/schemas/iso19139.napec/strings/Additionalinformation"/></h3>


      <xsl:call-template name="showPanel">
        <xsl:with-param name="title"   select="/root/gui/schemas/iso19139.napec/strings/Metadatarecord"/>
      ...
      <xsl:call-template name="showPanel">
        <xsl:with-param name="title"   select="/root/gui/schemas/iso19139.napec/strings/Datasetidentification"/>
        <xsl:with-param name="content">
          <table>
            <tbody>
  ...
  <xsl:template name="iso19139.napecFullView">
    <xsl:param name="schema"/>
    <xsl:param name="edit"/>
    ...
    <div class="col-md-8 ec-md-detail" itemscope="" itemtype="http://schema.org/Dataset">
      ...
      <xsl:call-template name="showPanel">
        ...
        <xsl:with-param name="title"   select="/root/gui/schemas/iso19139.napec/labels/element[@name='gmd:identificationInfo']/label"/>
        <xsl:with-param name="content">
      ...
      <xsl:if test="gmd:contentInfo/*">
        <xsl:call-template name="showPanel">
          <xsl:with-param name="title"   select="/root/gui/schemas/iso19139.napec/labels/element[@name='gmd:contentInfo']/label"/>
          <xsl:with-param name="content">
            <table class="sidebar2" style="table-layout:fixed; width:99% !important; word-wrap: break-word;">
              <tbody>
      ...
      <xsl:if test="gmd:distributionInfo/*/*[name() = 'gmd:distributionFormat']">
        <xsl:call-template name="showPanel">
          <xsl:with-param name="title"   select="/root/gui/schemas/iso19139.napec/labels/element[@name='gmd:distributionInfo']/label"/>
          <xsl:with-param name="content">
            <table class="sidebar2" style="table-layout:fixed; width:99% !important; word-wrap: break-word;">
              <tbody>
      ...
      <xsl:if test="gmd:dataQualityInfo/*">
        <xsl:call-template name="showPanel">
          <xsl:with-param name="title"   select="/root/gui/schemas/iso19139.napec/labels/element[@name='gmd:dataQualityInfo']/label"/>
          <xsl:with-param name="content">
            <table class="sidebar2" style="table-layout:fixed; width:99% !important; word-wrap: break-word;">
              <tbody>
      ...
      <xsl:if test="gmd:portrayalCatalogueInfo/*">
        <xsl:call-template name="showPanel">
          <xsl:with-param name="title"   select="/root/gui/schemas/iso19139.napec/labels/element[@name='gmd:portrayalCatalogueInfo']/label"/>
          <xsl:with-param name="content">
            <table class="sidebar2" style="table-layout:fixed; width:99% !important; word-wrap: break-word;">
              <tbody>
      ...
      <xsl:if test="gmd:metadataConstraints/*">
        <xsl:call-template name="showPanel">
          <xsl:with-param name="title"   select="/root/gui/schemas/iso19139.napec/labels/element[@name='gmd:metadataConstraints']/label"/>
          <xsl:with-param name="content">
            <table class="sidebar2" style="table-layout:fixed; width:99% !important; word-wrap: break-word;">
              <tbody>
      ...
      <xsl:if test="gmd:applicationSchemaInfo/*">
        <xsl:call-template name="showPanel">
          <xsl:with-param name="title"   select="/root/gui/schemas/iso19139.napec/labels/element[@name='gmd:applicationSchemaInfo']/label"/>
          <xsl:with-param name="content">
            <table class="sidebar2" style="table-layout:fixed; width:99% !important; word-wrap: break-word;">
              <tbody>
      ...
      <xsl:if test="gmd:metadataMaintenance/*">
        <xsl:call-template name="showPanel">
          <xsl:with-param name="title"   select="/root/gui/schemas/iso19139.napec/labels/element[@name='gmd:metadataMaintenance']/label"/>
          <xsl:with-param name="content">
            <table class="sidebar2" style="table-layout:fixed; width:99% !important; word-wrap: break-word;">
              <tbody>
      ...
      <xsl:if test="gmd:metadataExtensionInfo/*">
        <xsl:call-template name="showPanel">
          <xsl:with-param name="title"   select="/root/gui/schemas/iso19139.napec/labels/element[@name='gmd:metadataExtensionInfo']/label"/>
          <xsl:with-param name="content">
            <table class="sidebar2" style="table-layout:fixed; width:99% !important; word-wrap: break-word;">
              <tbody>
  ...
  <xsl:template name="sidebar-panel">
    ...
    <div class="col-md-4 row-end ec-md-detail mrgn-tp-sm">
      ...
      <xsl:if test="/root/gmd:MD_Metadata//gmd:graphicOverview/gmd:MD_BrowseGraphic/gmd:fileName/gco:CharacterString!=''">
        ...
        <xsl:if test="/root/gmd:MD_Metadata//gmd:graphicOverview/gmd:MD_BrowseGraphic/gmd:fileName/gco:CharacterString=$fileName">
          <xsl:call-template name="showPanel">
            ...
            <xsl:with-param name="title"   select="/root/gui/schemas/iso19139.napec/strings/Thumbnail"/>

            <xsl:with-param name="content">
              <xsl:choose>
                <xsl:when test="starts-with($fileName, 'http')">
                  <img class="ec-thumbnail" src="{$fileName}" itemprop="image" alt="{/root/gui/schemas/iso19139.napec/strings/Thumbnail}" title="{/root/gui/schemas/iso19139.napec/strings/Thumbnail}" />
                </xsl:when>
                <xsl:otherwise>
                  <img class="ec-thumbnail" src="{concat(/root/gui/url,'/srv/eng/resources.get?uuid=', /root/gmd:MD_Metadata//gmd:fileIdentifier/gco:CharacterString, '&amp;fname=', $fileName, '&amp;access=public')}" itemprop="image" alt="{/root/gui/schemas/iso19139.napec/strings/Thumbnail}" title="{/root/gui/schemas/iso19139.napec/strings/Thumbnail}" />
                </xsl:otherwise>
              </xsl:choose>
            </xsl:with-param>
          </xsl:call-template>
      ...
      <xsl:call-template name="showPanel">
        <xsl:with-param name="title"   select="/root/gui/schemas/iso19139.napec/strings/Dataclassification"/>
        <xsl:with-param name="content">
          <table style="border:none; table-layout:fixed; width:99% !important; word-wrap: break-word;">

            <!-- keywords -->
            <xsl:variable name="kCodelist" select="/root/gui/schemas/iso19139.napec/codelists/codelist[@name='gmd:MD_KeywordTypeCode']" />
  ...
  <xsl:template name="common-detailview-fields">
    ...
    <xsl:for-each select="/root/gmd:MD_Metadata//gmd:extent//gmd:EX_GeographicBoundingBox|/root/gmd:MD_Metadata//srv:extent//gmd:EX_GeographicBoundingBox">
      ...
      <div style="float:left;text-align:left;width:200px">
        ...
        <xsl:if test="$hasGeometry">
          <span class="bold"><xsl:value-of select="/root/gui/schemas/iso19139.napec/strings/GeographicExtent"/></span><br/>
          <span itemprop="spatial">
            <xsl:value-of select="/root/gui/schemas/iso19139.napec/strings/sw"/>: <xsl:value-of select="concat(format-number(gmd:westBoundLongitude/gco:Decimal,'#.##'), ' ', format-number(gmd:southBoundLatitude/gco:Decimal,'#.##'))"/> <br/>
            <xsl:value-of select="/root/gui/schemas/iso19139.napec/strings/ne"/>: <xsl:value-of select="concat(format-number(gmd:eastBoundLongitude/gco:Decimal,'#.##'), ' ', format-number(gmd:northBoundLatitude/gco:Decimal,'#.##'))"/>
          </span>
        </xsl:if>

        <!-- GeographicScope -->
        <xsl:if test="/root/gmd:MD_Metadata//gmd:keyword[../gmd:thesaurusName/gmd:CI_Citation/@id = 'local.place.EC_Geographic_Scope']">
          <br/><br/><span class="bold"><xsl:value-of select="/root/gui/schemas/iso19139.napec/strings/GeographicScope"/></span><br/>
        ...
        <xsl:if test="/root/gmd:MD_Metadata//gmd:temporalElement">
          <xsl:for-each select="/root/gmd:MD_Metadata//gmd:temporalElement/gmd:EX_TemporalExtent/gmd:extent/gml:TimePeriod">
            ...
            <br/><br/><span class="bold"><xsl:value-of select="/root/gui/schemas/iso19139.napec/strings/Timeperiod"/></span><br/>
            <xsl:value-of select="/root/gui/schemas/iso19139.napec/strings/from"/>: <span itemprop="temporal"><xsl:value-of select="gml:beginPosition"/></span> <br/>
            <xsl:value-of select="/root/gui/schemas/iso19139.napec/strings/to"/>:  <span itemprop="temporal"><xsl:value-of select="gml:endPosition"/></span>
          </xsl:for-each>
        </xsl:if>
      </div>
    </xsl:for-each>
    ...
    <xsl:if test="$mapResourcesCount > 0">
      <!-- MAP RESOURCES -->
      <h3><xsl:value-of select="/root/gui/schemas/iso19139.napec/strings/MapResources"/></h3>
    ...
    <xsl:if test="($resourcesCount - $mapResourcesTotalCount) > 0">
      <h3><xsl:value-of select="/root/gui/schemas/iso19139.napec/strings/Dataresources"/></h3>
    ...
    <xsl:for-each-group select="/root/gmd:MD_Metadata/gmd:distributionInfo/gmd:MD_Distribution/gmd:transferOptions/gmd:MD_DigitalTransferOptions/gmd:onLine/gmd:CI_OnlineResource[lower-case(normalize-space(gmd:protocol/gco:CharacterString))=$webMapServicesProtocols/record/name]"
      ...
      <table class="wb-tables table table-striped table-hover" style="table-layout:fixed; width:99% !important; word-wrap: break-word;">
        ...
        <thead style="display: none">
          <th style="width: 65%"><xsl:value-of select="/root/gui/schemas/iso19139.napec/strings/Dataresources_Name" /></th>
          <th style="width: 35%"><xsl:value-of select="/root/gui/schemas/iso19139.napec/strings/Dataresources_Link" /></th>
        </thead>
  ...
  <xsl:template name="data-resources">
    ...
    <xsl:for-each-group select="/root/gmd:MD_Metadata/gmd:distributionInfo/gmd:MD_Distribution/gmd:transferOptions/gmd:MD_DigitalTransferOptions/gmd:onLine/gmd:CI_OnlineResource[not(lower-case(normalize-space(gmd:protocol/gco:CharacterString))=$webMapServicesProtocols/record/name)]"
      ...
      <table class="wb-tables table table-striped table-hover" style="table-layout:fixed; width:99% !important; word-wrap: break-word;">
        ...
        <thead style="display: none">
          <th style="width: 75%"><xsl:value-of select="/root/gui/schemas/iso19139.napec/strings/Dataresources_Name" /></th>
          <th style="width: 25%"><xsl:value-of select="/root/gui/schemas/iso19139.napec/strings/Dataresources_Link" /></th>
        </thead>
